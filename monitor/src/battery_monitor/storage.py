...
class RetentionStore:
    ...
    def insert_snapshot(self, snapshot: dict) -> int:
        ...
        now_iso = _iso_from_unix(now_unix)
        rows = []
            ...
            rows.append(
                {
                    ...
                    "address": int(battery.get("address", reading.get("address", 0)) or 0),
    ...
    def latest_states(self) -> list[dict]:
        cursor = self.connection.execute(
            """
            SELECT raw_json
            FROM readings
        ...
        for row in cursor.fetchall():
            try:
                states.append(json.loads(row["raw_json"]))
    ...
    def events(self, seconds: int, limit: int) -> list[dict]:
        since = int(time.time()) - seconds
        cursor = self.connection.execute(
            """
    ...
    def export_rows(self, battery_id: str, days: int) -> list[dict]:
    ...
    def stats(self, retention_days: int) -> dict:
        cursor = self.connection.execute(
            """
